...
import defaultAvatar from '../assets/react.svg'; // Đảm bảo đường dẫn ảnh đúng
...
const Navbar = () => {
    ...
    // Xử lý chuyển hướng đến trang login (lưu lại trang hiện tại để redirect về sau khi login)
    ...
    // Nếu đang load user, có thể return null hoặc render skeleton. 
    // Ở đây ta cứ render Header bình thường để tránh layout shift, Auth sẽ cập nhật sau vài tích tắc.
        ...
        <header className="w-full bg-white shadow-md sticky top-0 z-50 font-sans">
            ...
            <div className="bg-blue-600 text-white text-xs py-2">
                <div className="container mx-auto px-4 flex justify-between items-center">
                    <p className="hidden md:block">Chào mừng đến với ElectroShop - Hàng chính hãng 100%</p>
                    <div className="flex items-center gap-4 mx-auto md:mx-0">
                        <span className="flex items-center gap-1 hover:text-gray-200 cursor-pointer">
                            <Phone size={14} /> Hotline: 1900 1234
                        </span>
                        <span className="hover:text-gray-200 cursor-pointer">Tra cứu đơn hàng</span>
                    </div>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-4">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        {/* --- USER ACCOUNT DROPDOWN (LOGIC TÍCH HỢP) --- */}
                        <div className="relative group z-20">
                            <div className="flex flex-col items-center cursor-pointer pb-2 md:pb-0">
                                {/* Nếu có User thì hiện Avatar, không thì hiện Icon User */}
                                {currentUser ? (
                                ...
                                <span className="text-xs text-gray-500 mt-1 group-hover:text-blue-600 flex items-center gap-0.5 max-w-[100px] truncate">
                                    {currentUser ? currentUser.fullname : "Tài khoản"} <ChevronDown size={10} />
                                </span>
                            </div>

                            {/* Dropdown Menu Content */}
                            <div className="absolute right-0 top-full mt-0 w-56 bg-white rounded-lg shadow-xl border border-gray-100 hidden group-hover:block animate-in fade-in slide-in-from-top-2 duration-200">
                                {currentUser ? (
                                    ...
                                    <div className="py-2">
                                        ...
                                        <div className="p-1">
                                            ...
                                            {currentUser.role === 'Admin' && (
                                                <Link to="/admin/dashboard" className="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md transition-colors">
                                                    <LayoutDashboard size={16} /> Quản trị (Admin)
                                                </Link>
                                            )}
                                            
                                            <Link to="/profile" className="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors">
                                                <UserCircle size={16} /> Tài khoản của tôi
                                            </Link>
                                            <Link to="/orders" className="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors">
                                    ...
                                    <div className="p-3 space-y-2">
                                        <button onClick={handleLoginClick} className="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors">
                                            <LogIn size={16} /> Đăng nhập
                                        </button>
                                        <Link to="/register" className="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors">
            ...
            {/* --- 3. NAVIGATION MENU --- */}
            <nav className={`bg-gray-50 border-t border-gray-200 transition-all duration-300 ${isMobileMenuOpen ? 'block' : 'hidden md:block'}`}>
                <div className="container mx-auto px-4">
                    <ul className="flex flex-col md:flex-row md:items-center md:gap-8 py-2 md:py-0">
                            ...
                            {currentUser ? (
                                <div className="space-y-3">
                                    <div className="flex items-center gap-3 p-2 bg-blue-50 rounded-lg">
                                        <img src={currentUser.avatar || defaultAvatar} alt="avt" className="w-10 h-10 rounded-full object-cover"/>
                                        <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <Link to="/profile" className="text-center py-2 border border-gray-300 rounded-md text-sm font-medium hover:bg-gray-50">Hồ sơ</Link>
                                        {currentUser.role === 'Admin' && (
                                            <Link to="/admin/dashboard" className="text-center py-2 border border-red-200 text-red-600 rounded-md text-sm font-medium hover:bg-red-50">Admin</Link>
                                        )}
                                    </div>
                                    <button 
                                        onClick={handleLogout}
                                ...
                                <div className="flex items-center gap-3 space-y-2">
                                    <button onClick={handleLoginClick} className="flex-1 text-center py-2 bg-blue-100 text-blue-700 rounded-md font-medium text-sm">
                                        Đăng nhập
                                    </button>
                                    <Link to="/register" className="flex-1 text-center py-2 bg-blue-600 text-white rounded-md font-medium text-sm">
                                        Đăng ký
                                    </Link>
                                </div>
                            )}
                        ...
                        <li className="py-2 md:py-3 border-b md:border-none">
                            <Link to="/products" className="font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-2">
