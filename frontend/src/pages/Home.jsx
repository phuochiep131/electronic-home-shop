...
import {
  ...
  Monitor,
  ChevronDown,
  Flame, // Import thêm icon Flame (lửa)
} from "lucide-react";
            ...
            <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4 border-b border-white/20 pb-4">
              <div className="flex items-center gap-4">
                <h2 className="text-2xl md:text-4xl font-black italic flex items-center gap-2 uppercase tracking-tighter">
                  <Zap className="text-yellow-300 fill-current animate-pulse" size={32} /> FLASH SALE
                </h2>
                
                {/* Đồng hồ đếm ngược kiểu Glassmorphism */}
                <div className="flex items-center gap-2 bg-black/20 backdrop-blur-md px-4 py-2 rounded-xl text-sm md:text-base font-bold border border-white/10 shadow-inner">
                  <Clock className="text-yellow-300" size={18} />
                  {timeLeft.days > 0 && (
                    <><span className="text-yellow-300">{timeLeft.days}d</span> : </>
                  )}
                  <span className="bg-white/20 px-2 rounded min-w-[30px] text-center">{String(timeLeft.hours).padStart(2, "0")}</span> :
                  <span className="bg-white/20 px-2 rounded min-w-[30px] text-center">{String(timeLeft.minutes).padStart(2, "0")}</span> :
                  <span className="bg-white/20 px-2 rounded min-w-[30px] text-center">{String(timeLeft.seconds).padStart(2, "0")}</span>
                </div>
              </div>
              ...
              {flashSaleProducts.map((item) => {
                 ...
                 return (
                  <Link
                    ...
                    key={item._id}
                    className="bg-white text-gray-800 rounded-xl p-3 hover:shadow-2xl transition-all transform hover:-translate-y-1 relative group block"
                  >
                    {/* Badge giảm giá */}
                    ...
                    <div className="w-full h-32 md:h-40 mb-3 overflow-hidden rounded-lg bg-gray-50 flex items-center justify-center p-2">
                        <img
                    ...
                    <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden border border-gray-100">
                      ...
                      {percentSold >= 90 && (
                         <div className="absolute right-0 top-0 h-full flex items-center pr-1">
                             <Flame className="text-yellow-200 fill-current animate-pulse" size={10} />
                         </div>
                      )}
                    </div>
                  </Link>
