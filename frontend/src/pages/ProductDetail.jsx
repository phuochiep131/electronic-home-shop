...
const ProductDetail = () => {
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            try {
                ...
                const img = data.image_url || "https://placehold.co/600x600/png?text=No+Image";
                setMainImage(img);

                // 2. Lấy sản phẩm tương tự (cùng danh mục)
                if (data.category_id && data.category_id._id) {
                    const catId = data.category_id._id;
                    const relatedRes = await axios.get(`${API_URL}/products?category=${catId}`);
    ...
    const specsList = [
        { label: "Danh mục", value: product.category_id?.category_name },
        ...
        <div className="bg-gray-50 min-h-screen py-8 font-sans">
            <div className="container mx-auto px-4">
                ...
                <div className="text-sm text-gray-500 mb-6 flex items-center gap-2 overflow-x-auto whitespace-nowrap pb-2 md:pb-0">
                    ...
                    <span>/</span>
                    <Link to="#" className="hover:text-blue-600">{product.category_id?.category_name || "Sản phẩm"}</Link>
                    <span>/</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 md:p-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="aspect-square bg-gray-100 rounded-xl overflow-hidden border border-gray-200 relative group">
                                <img
                                    src={mainImage || "https://placehold.co/600x600/png?text=No+Image"}
                        ...
                        <div>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-4 text-sm">
                                        <div className="flex items-center text-yellow-400">
                                            {[...Array(5)].map((_, i) => (
                                                <Star key={i} size={16} fill={i < Math.floor(ratingMock) ? "currentColor" : "none"} className={i >= Math.floor(ratingMock) ? "text-gray-300" : ""} />
                                            ))}
                                            <span className="font-bold text-gray-900 ml-2">{ratingMock}</span>
                                        </div>
                                        <span className="text-gray-300">|</span>
                                        <span className="text-blue-600 hover:underline cursor-pointer">{reviewsMock} đánh giá</span>
                                        <span className="text-gray-300">|</span>
                                        <span className="text-gray-500">Mã SP: {product._id.slice(-6).toUpperCase()}</span>
                                    </div>
                                </div>
                            ...
                            <div className="bg-blue-50/50 p-4 rounded-xl mb-6 border border-blue-100">
                                <div className="flex items-end gap-3 mb-1">
                                    ...
                                    {discount > 0 && (
                                        <span className="text-gray-400 line-through text-lg mb-1">{formatCurrency(originalPrice)}</span>
                                    )}
                                </div>
                                {discount > 0 && (
                                    <div className="text-sm text-blue-800 flex items-center gap-1 font-medium">
                                        <Tag size={14} /> Tiết kiệm: {formatCurrency(originalPrice - currentPrice)}
                                    </div>
                                )}
                            </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6 mt-8">
                                        <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                                            ...
                                            <ul className="space-y-2">
                                                <li className="flex items-start gap-2 text-sm"><span className="w-1.5 h-1.5 bg-gray-400 rounded-full mt-2"></span>Sản phẩm chính hãng chất lượng cao.</li>
                                                <li className="flex items-start gap-2 text-sm"><span className="w-1.5 h-1.5 bg-gray-400 rounded-full mt-2"></span>Bảo hành dài hạn, hỗ trợ kỹ thuật 24/7.</li>
                                                <li className="flex items-start gap-2 text-sm"><span className="w-1.5 h-1.5 bg-gray-400 rounded-full mt-2"></span>Thiết kế hiện đại, phù hợp mọi không gian.</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <table className="w-full text-sm text-left text-gray-600">
                                            <tbody>
                                                {specsList.length > 0 ? (
                                                    ...
                                                    <tr>
                                                        <td className="py-4 px-6 text-center text-gray-500" colSpan="2">Chưa có thông số kỹ thuật</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>
                ...
                <div className="mt-12">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link to="/products" className="text-blue-600 font-medium hover:underline flex items-center gap-1 text-sm">
                            Xem tất cả
                        </Link>
                    </div>
                    ...
                    {relatedProducts.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                            {relatedProducts.map((item) => {
                                // Tính giá sản phẩm tương tự
                                const itemOriginalPrice = item.price;
                                const itemDiscount = item.discount || 0;
                                const itemCurrentPrice = itemOriginalPrice * (1 - itemDiscount / 100);

                                return (
                                    <Link to={`/product/${item._id}`} key={item._id} className="group bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all cursor-pointer block h-full flex flex-col">
                                        <div className="aspect-square bg-gray-100 rounded-lg mb-3 overflow-hidden relative flex-shrink-0">
                                            <img 
                                                src={item.image_url || "https://placehold.co/300x300/png?text=Product"} 
                                        ...
                                        <div className="text-xs text-gray-500 mb-1">{item.category_id?.category_name || "Sản phẩm"}</div>
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            <div className="flex flex-col">
                                                <span className="text-blue-600 font-bold text-sm">{formatCurrency(itemCurrentPrice)}</span>
                                                {itemDiscount > 0 && (
                                                    <span className="text-xs text-gray-400 line-through">{formatCurrency(itemOriginalPrice)}</span>
                                                )}
                                            </div>
                        ...
                        <div className="text-center text-gray-500 py-8">Không có sản phẩm tương tự nào.</div>
                    )}
                </div>
            </div>
        </div>
